...
export default function Header() {
  ...
  return (
    <header
      className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${isScrolled ? "bg-white shadow-md py-2" : "bg-white/95 backdrop-blur-md py-4 border-b border-gray-100"}`}
    >
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between">
          ...
          >
            ...
            <div className="font-heading font-bold text-xl text-charcoal">
              <span className="text-earth-green relative">E</span>Thix
            </div>
          </Link>
          ...
          <div className="flex items-center space-x-6">
            <nav className="hidden md:block h-full">
              <ul className="flex items-center space-x-8 h-full">
                {navLinks.map((link, index) => (
                  <li
                    key={index}
                    className={`${link.isDropdown ? "relative group" : ""}`}
                  >
                    <Link
                      href={link.href}
            ...
            <Link
              href="/get-started"
              className="hidden md:inline-flex items-center justify-center rounded-full bg-earth-green hover:bg-earth-green-dark text-white font-medium py-2 px-5 transition-colors duration-200 cursor-pointer"
            >
        ...
        >
          <div className="px-2 pt-2 pb-3 space-y-1 border-t border-gray-100">
            ...
            <Link
              href="/get-started"
              className="flex items-center justify-center w-full rounded-full bg-earth-green text-white font-medium py-3 px-5 mt-3 hover:bg-earth-green-dark transition-colors duration-200 cursor-pointer"
            >
              {t("navigation.get_started")}
              <svg
                xmlns="http://www.w3.org/2000/svg"
        ...
        <div
          className={`absolute inset-x-0 transition-all duration-300 bg-white shadow-md ${isSearchOpen ? "top-full opacity-100 pointer-events-auto" : "-top-full opacity-0 pointer-events-none"}`}
        >
          <div className="container py-4">
            <form className="flex items-center">
              ...
              <button
                type="submit"
                className="ml-2 rounded-full bg-earth-green hover:bg-earth-green-dark text-white font-medium py-3 px-5 transition-colors duration-200 cursor-pointer"
              >
                Search
              </button>
              <button
            ...
            <div className="mt-4">
              <h4 className="text-xs font-medium text-slate mb-2">
                POPULAR SEARCHES:
              </h4>
              <div className="flex flex-wrap gap-2">
                <Link
                  href="/search?q=eco-friendly"
                  className="px-3 py-1 text-xs bg-earth-green/10 text-earth-green rounded-full hover:bg-earth-green/20 transition-colors cursor-pointer"
                >
                  Eco-friendly products
                </Link>
                <Link
                  href="/search?q=sustainable-fashion"
                  className="px-3 py-1 text-xs bg-earth-green/10 text-earth-green rounded-full hover:bg-earth-green/20 transition-colors cursor-pointer"
                >
                  Sustainable fashion
                </Link>
                <Link
                  href="/search?q=fair-trade"
                  className="px-3 py-1 text-xs bg-earth-green/10 text-earth-green rounded-full hover:bg-earth-green/20 transition-colors cursor-pointer"
                >
                  Fair trade
                </Link>
                <Link
                  href="/search?q=carbon-neutral"
                  className="px-3 py-1 text-xs bg-earth-green/10 text-earth-green rounded-full hover:bg-earth-green/20 transition-colors cursor-pointer"
                >
                  Carbon neutral
                </Link>
